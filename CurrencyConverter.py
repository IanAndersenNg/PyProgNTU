...
class CurrencyConverter(customtkinter.CTkToplevel):
    def __init__(self, *args, **kwargs):
        ...
        def convert_currency():
            try:
                ...
                result_label.configure(text="{:.2f} {} = {:.2f} {}".format(amount, from_currency, result, to_currency))
        ...
        def convert_total_expense():
            try:
                ...
                expense_result_label.configure(text="{:.2f} USD = {:.2f} {}".format(total_expense, result, to_currency))

            except ValueError:
                messagebox.showerror("Input Error", "Please enter a valid number.")

        # 创建主窗口
        self.title("Currency Converter")  # 主窗口标题
        customtkinter. set_appearance_mode("Dark")
        # 创建主框架
        main_frame = customtkinter.CTkFrame(self)
        main_frame.grid(row=1, column=1, padx = 50, pady = 20)


        # 创建“总支出”框架
        expense_frame = customtkinter.CTkFrame(main_frame)
        expense_frame.pack(padx=10, pady=10)


        # 总支出输入
        customtkinter.CTkLabel(expense_frame, text="Total Expenses (USD)",font=("Arial", 20)).grid(row=0, column=0, sticky=W, padx= 10)
        total_expense_entry = customtkinter.CTkEntry(expense_frame)
        total_expense_entry.grid(row=1,column=0, padx=5,pady=(10, 10))

        # 货币选择和转换按钮
        customtkinter.CTkLabel(expense_frame, text="Convert to Currency",font=("Arial", 20)).grid(row=0, column=3, padx=(10, 10),pady=5)
        ...
        convert_expense_button = customtkinter.CTkButton(expense_frame, text="Calculate", command=convert_total_expense,width=15)
        convert_expense_button.grid(row=2, column=0, sticky=tk.EW, padx=(10, 10), pady=(10, 10))

        expense_result_label = customtkinter.CTkLabel(expense_frame, text="Total Expenses Conversion Result")
        expense_result_label.grid(row=2, column=3, sticky=EW, padx=(10, 0), pady=(10, 10))

        # 创建“汇率展示”框架
        exchange_rates_frame = customtkinter.CTkFrame(main_frame)
        exchange_rates_frame.pack(pady=5, fill=tk.BOTH, expand=False)

        # “汇率展示”标题
        title_label = customtkinter.CTkLabel(exchange_rates_frame, text="Current Exchange Rates", font=("Arial", 16))
        title_label.pack(pady=5)

        # 创建滚动条框架
        table_frame = customtkinter.CTkFrame(exchange_rates_frame)
        ...
        # 创建“汇率计算”框架
        conversion_frame = customtkinter.CTkFrame(main_frame)
        conversion_frame.pack(padx=50, pady=20)

        # 下拉列表和按钮布局
        currency_row = customtkinter.CTkFrame(conversion_frame)
        currency_row.grid(row=0, column=0, columnspan=3, padx=10, pady=10, sticky="ew")

        customtkinter.CTkLabel(currency_row, text="From Currency:").grid(row=0, column=0, sticky=tk.W, padx=10)
        major_currencies = ["USD", "EUR", "JPY", "GBP", "AUD", "CAD", "CHF", "CNY", "SEK", "NZD",
                            "MXN", "SGD", "HKD", "NOK", "KRW", "TRY", "RUB", "INR", "BRL", "ZAR"]
        major_currencies_sorted = sorted(major_currencies)
        ...
        customtkinter.CTkLabel(currency_row2, text="To Currency:").grid(row=0, column=0, sticky=tk.W, padx=10)
        major_currencies = ["USD", "EUR", "JPY", "GBP", "AUD", "CAD", "CHF", "CNY", "SEK", "NZD",
                            "MXN", "SGD", "HKD", "NOK", "KRW", "TRY", "RUB", "INR", "BRL", "ZAR"]
        major_currencies_sorted = sorted(major_currencies)
        ...
        amount_row = customtkinter.CTkFrame(conversion_frame)
        amount_row.grid(row=2, column=0, columnspan=4, padx=10, pady=10, sticky="ew")

        customtkinter.CTkLabel(amount_row, text="Amount:").grid(row=0, column=0, sticky=tk.W, padx=10)
        amount_entry = tk.Entry(amount_row)
        amount_entry.grid(row=0, column=1, sticky=tk.EW, padx=(48,0))

        result_row = customtkinter.CTkFrame(conversion_frame)
        result_row.grid(row=3, column=0, columnspan=4, padx=10,pady=10, sticky="ew")

        result_label = customtkinter.CTkLabel(result_row, text="Expenses Conversion Result")
