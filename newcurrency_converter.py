...
# 创建“总支出”框架
expense_frame = tk.Frame(currency_converter_frame)
expense_frame.pack(pady=10, fill=tk.X, expand=False)

# 总支出输入
tk.Label(expense_frame, text="Total Expense (USD):").grid(row=0, column=0, padx=5, pady=5, sticky='e')
total_expense_entry = tk.Entry(expense_frame)
total_expense_entry.grid(row=0, column=1, padx=5, pady=5)

# 货币选择和转换按钮
tk.Label(expense_frame, text="Convert to Currency:").grid(row=0, column=2, padx=5, pady=5, sticky='e')
...
expense_currency_combobox.grid(row=0, column=3, padx=5, pady=5)
expense_currency_combobox.set("EUR")

convert_expense_button = tk.Button(expense_frame, text="Convert Total Expense", command=convert_total_expense)
convert_expense_button.grid(row=1, column=0, columnspan=2, pady=5)

expense_result_label = tk.Label(expense_frame, text="Total Expense Conversion Result")
expense_result_label.grid(row=1, column=2, columnspan=2, padx=5, pady=5)
...
# 创建退出按钮
exit_button = tk.Button(main_frame, text="Exit", command=lambda:cc.destroy())
